...
function create_container(){
    if [ ! -f "$containerFile" ];then
        sudo docker stop $containerName
        sudo docker rm $containerName
        sudo docker run -itd --name $containerName --restart=always --privileged=true -p 8083:8083 -p 5678:5678 -v /system:/system -v /etc/localtime:/etc/localtime:ro -v /etc/timezone:/etc/timezone:ro -e PYTHONPATH=/system/lib $imageName  /bin/bash /etc/init.d/start_server.sh 
        if [ $? -eq 0 ];then
